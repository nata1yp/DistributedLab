...
class Transaction:
    ...
    def outputUTXOS(self, receiverID, senderID):
        ...
        if mysum - self.amount > 0:
            outputs.append({'id': str(senderID), 'tid': self.transaction_id, 'amount': mysum - self.amount})
        outputs.append({'id': str(receiverID), 'tid': self.transaction_id, 'amount': self.amount})
        self.transaction_outputs = outputs;
